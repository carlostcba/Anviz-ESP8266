...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\nIniciando emulador de dispositivo Anviz");
...
void checkWiegandCard() {
  ...
  if (wiegandBitCount > 0 && millis() - wiegandTimeoutStart > WIEGAND_TIMEOUT) {
    ...
    if (wiegandBitCount == 26) { // Wiegand 26
      ...
      // Reorganizar bytes para formato estándar
      cardId = ((cardId >> 16) & 0xFF) | ((cardId & 0xFF) << 16) | (cardId & 0xFF00);
    } 
    ...
    Serial.print("Tarjeta detectada: 0x");
    ...
    if (userIndex >= 0) {
      // Usuario encontrado
      Serial.print("Acceso concedido a: ");
